...
cp Ad_Hoc__MissingChildrenMinnesota_Wildcard.mobileprovision $HOME/Library/MobileDevice/Provisioning Profiles/$(/usr/libexec/PlistBuddy -c "Print UUID" /dev/stdin <<< $(/usr/bin/security cms -D -i "Ad_Hoc__MissingChildrenMinnesota_Wildcard.mobileprovision")).mobileprovision
cp Dev__MissingChildrenMinnesota_Wildcard.mobileprovision $HOME/Library/MobileDevice/Provisioning Profiles/$(/usr/libexec/PlistBuddy -c "Print UUID" /dev/stdin <<< $(/usr/bin/security cms -D -i "Dev__MissingChildrenMinnesota_Wildcard.mobileprovision")).mobileprovision
